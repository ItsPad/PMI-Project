...
const StatsDisplay = ({ stats, isLoading }) => {
  ...
  if (stats.count === 0) {
    return (
       <div className="mb-6 p-4 bg-gray-50 rounded-lg text-center">
        <p className="text-gray-500">ยังไม่มีข้อมูลสำหรับคำนวณค่าเฉลี่ย 7 วัน</p>
      </div>
    );
  }
  ...
  return (
    <div className="mb-6 p-4 bg-gray-50 rounded-lg text-center">
      ...
      <p className="text-3xl font-bold text-gray-800">
        {stats.avgSys} / {stats.avgDia} <span className="text-lg font-normal">mmHg</span>
      </p>
...
const Dashboard = ({ profile, onLogout }) => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      if (data.length === 0) {
        setMessage({ type: 'success', text: '✅ เชื่อมต่อสำเร็จ! (ยังไม่มีข้อมูลให้แสดง)' });
      } else {
         setMessage({ type: '', text: '' }); 
      }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!systolic || !diastolic) { // 👈 ไม่บังคับให้กรอก feeling
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-2xl text-center">
        ...
        {isLoading && (
          <div className="mt-6 p-3 rounded-lg bg-blue-100 text-blue-700">
            กำลังโหลดข้อมูลจากเซิร์ฟเวอร์ กรุณารอสักครู่ใช้เวลาประมาณ 30 วินาที... 🔄
          </div>
        )}
